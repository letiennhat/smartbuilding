...
#    global name
    ...
    for i in range(dem_x) :
        ...
        for j in allfiles_1:
            ...
            try:  
                if not os.path.exists('variable/'+allfiles[i]+j+'.txt'):
                    ...
                    try:
                        ...
                        b=face_recognition.face_encodings(a,face_locations_1,num_jitters=1)[0]#known_face_locations=[[0,a.shape[1],a.shape[0],0]])[0]#,known_face_locations=[[0,a.shape(1),a.shape(0),0]])
...
#            filechuyencan1.close()
        ...
        for face_encoding in face_encodings:
            count_no_face=0
            threshold = 0.32198888888465666682982228299
        ...
        #                        open_ff.close()
            ...
            print("time_read_open",time.time()-begin_time)
            if name != "Unknow":
                pass
                #time.sleep(2)
            #time.sleep(1)
        ...
        cv2.imshow(sourceName, cv2.resize(frame, (0,0), fx=1/2, fy=1/2, interpolation=cv2.INTER_AREA))
